...
SOAP?} The SOAP search has been run on a number of observing runs~\chris{run on runs}
to search for \gls{CW}.  One of the major factors which~\chris{that} limited the sensitivity
...
\end{figure}
%
~\chris{Fig 5.2 straight after Fig 5.2 - clunky} Figure
...
instrumental lines themselves.~\chris{You have not explained why the line aware
statistic fails in this example case - you really should if you haven't
already. If you have then please refer the reader to the appropriate section.}
In this section, I will explain the setup of the search to identify
instrumental lines.

Whilst it is useful to run searches on auxiliary channels when trying to
...
sensitivity of a search for \glspl{CW}~\chris{you jump quickly into the
multiple detectors argument but it's confusing becuase it's straight after the
multiple channels discussion. For multi-channel data you would also want to
consider using it just like you do for multi-detector. However, you next
statement is that you don't want to do this here.}. However, when searching for
instrumental lines, the aim is to have the reverse~\chris{is it really the
"reverese" effect?} effect, therefore, a simple search is run separately on
each detector.  This then removed any~\chris{tenses are all over the place
here} of the statistics developed in Sec.~\ref{soap:las} and uses the
`normalised' \gls{SFT} power as the statistic in the SOAP search~\chris{here
you only refer to a previous section but I think you should specifically point
to the equation that defines your statistc in this case. Plus you should define
any fixed or tuned parameters if there are any - or is it only the transition
matrix?}. The single
...
straight or down a frequency bin~\chris{maybe a bit of puncuation in this last
sentence}.  In this search we are aiming to find any~\chris{not really any, but
line-like} non Gaussian artefacts,
...
should be identifiable in the raw 1800s long \glspl{SFT}~\chris{possible
examiner question - what would happen if you did sum the data and do the line
search on summed power?}. The Fscan search
described above generates \glspl{SFT} every day of varying lengths~\chris{super
ambiguous - it's like Fscan randomply churns out whatever SFT lengths it feesl
like}, and this
includes 1800 s long \glspl{FFT}.  For this line search, we split the 1800 s
long Fscan \glspl{FFT} into 0.2 Hz~\chris{why 0.2Hz? Didn't you use 0.1Hz for
the search. Please provide motivation for this choice.} wide sub-bands and run the single detector
...
\end{figure}
%
Fig.~\ref{detchar:soap:noiseplot}~\chris{You can't start a sentence with the
abreviated version of Figure or Section or Equation. It has to be the full
version} shows an example of the spectrogram of a
...
~\chris{astrophysical?} signal
present,~\chris{comma not really suitable here} this is because the track appears to be randomly wandering and has a
...
Fig.~\ref{detchar:soap:lineplot}~\chris{no abreviation of Fig here}
demonstrates the same figure as before~\chris{you can't say things like "the
same figure as before" - refer specifically to the relavent figure.} but now with a strong and narrow
...
sub-band~\chris{repetetive of the previous sentence}. The track
...
the track~\chris{why "around" and not "on" the track?} which are of high log-probability. The areas of white in the Viterbi
...
that there is a narrow spectral artefact within the sub-band.~\chris{in all of
these plot descriptions you do not refer to the lower panel. The track power
can tell you have stable the signal was in amplitude and can certainly help to
rule out an astrophysical signal. Something to mention?} 
...
Fig.~\ref{detchar:soap:wanderplot} once again~\chris{mbiguous language - this
figure hasn't shown anything before} shows the equivalent plots to
Fig.~\ref{detchar:soap:noiseplot} and \ref{detchar:soap:lineplot} but now
contains some~\chris{informal - a} wandering spectral artefact.  This is
some~\chris{informal} line which wanders in
frequency as is moves though the band~\chris{moving through the band is the
same as wandering in frequency - you mean moves through time or equivelent}.  This can be seen in the frequency
track, which here does not have much spread, however, the frequency of the
potential signal~\chris{what is a potential signal? just use track} changes with time.  There are also areas where the signal
could have potentially~\chris{potential signal and now the signal could have
potentially switched} switched to a separate spectral artefact within the same
...
used to flag this~\chris{which sub-band are we talking about?} sub-band as
containing some sort of~\chris{too informal - a possible} instrumental
artefact.  The example plots here~\chris{where is here?} show some specific
...
most likely from an instrumental artefact~\chris{you have to be very careful
saying things like this. Saying something is the "most likely" is a very clear
probabilistic statement however you have provided no formal proof that this is
the case. You should remove this statement but you shoiuld also explain why you
think that high Vit stat values would be good indicators for the presence of a
line artefact}. Figure~\ref{detchar:soap:rankedstats} shows an example of a
histogram of the ranked statistics~\chris{which statitics? Vit stat? Also the
plot is missing - I assume this is the double mode distribution you were taling
about today.} from O2. The sub-bands which give statistic
