...
statistics and is where some flexibility is allowed~\chris{I wouldn't word it
this way - plus I wouldn't lead the description with it.}. However, this is a distribution of the parameters ${\bm \theta}$ of the model $I$ which should reflect any beliefs prior to the observations.
...
the Bayesian Evidence in Eq.~\ref{searchcw:bayes:evidence},~\chris{not a comma
location} this involves calculating an
integral over all possible parameters.  There is however, a way around having
to calculate this~\chris{if you're only interested in the posterior}.  For any given model $I$, the Bayesian Evidence $p({\bm d}\mid I)$ is
...
as~\chris{represented by?} the points in
...
can draw it~\chris{don't use the word draw in this sense since "draw" also
means to generate samples.} over these points.  For each of these points, we can calculate the
...
distribution and ignored~\chris{you forgot to mention that the original 2D
samples you make have to have a maximum at least as high as the target
distribution maximum}. The values of $\theta$ where $p_i < p(\theta_i \mid
...
In practice we are interested in areas of high probability, i.e. the larger
values of the posterior, as this will correlate with the truth~\chris{difficult
to grasp the meaning of this sentence}.  Therefore, a method titled \gls{MCMC}
can be used to concentrate the samples around areas of high probability,
approximating the posterior distribution more efficiently,~\chris{you love
misusing commas} more information on
...
This~\chris{new paragraph so you can't refer to "This" - what is "This"?}
...
values of this point are stored~\chris{stored for what?}.  If the posterior value is lower than the
...
posterior in uninteresting areas of parameter space~\chris{well this isn't the
case unless you define a jump proposal
distribution. In the text here you don't mention it but it is in the algorithm
as a normal. The not wateing time part is only due to the fact that the juymp
proposal makes it hard to jump very far from where you currently are}.  
...
\mid \bm{d})$. Here $\mathcal{N}$ is some distribution~\chris{this is usually
reserved to refer to a Normal distribution not an artibtrary one} which describes the jump
...
Bayesian Evidence. \gls{MCMC} methods can estimate this~\chris{can they? Not
the masic versions. Give references?}, however they are inefficient, and therefore a method known as Nested
sampling was developed to solve this problem
\citep{skilling2006NestedSampling,speagle2019DynestyDynamic}.~\chris{bit clunky
this paragraph. We need the evidence to make the bayes factor and therefore we
need the evidence. Also, you don't actually say in words what the bayes factor
literally is. You say compare models but then go no further.}
...
limits are defined by the likelihood~\chris{what limits? I dont understand this
statement}.
...
in Eq.~\ref{searchcw:bayes:nested:evidence}.~\chris{very nice but I would
recommend using a higher value of lambda for this plot because it's hard to see
the upper boundary of the blue area. 0.25 would be better.}}
...
\end{equation}
%
~\chris{can you explain how you get this equation? It's based on the
probability of the lowest value of X at each step}
...
increasingly smaller prior masses.~\chris{I don't understand the second while
loop. What is it supposed to be doing?} \label{searchcw:bayes:nested_alg}}
\end{algorithm}
%
...
\end{equation}
where $\delta^{\theta}_{\theta_i}$ is the Dirac~\chris{well it looks llike the
Kronecker to me. Best to use the notation $\delta(\theta-\theta_{i})$.} delta
