...
Sampling a distribution, say the posterior $p(\bm{\theta} \mid \bm{d})$, means choosing a value (or sample) of the parameters $\bm{\theta}$ such that if we chose many independent samples of $\bm{\theta}$ the number within any range $\bm{\theta} \rightarrow \bm{\theta} + \delta \bm{\theta}$ is proportional to the height of the distribution $p(\bm{\theta} \mid \bm{d})$.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		%
	\end{algorithmic}
	\caption[Nested sampling algorithm]{ Nested sampling algorithm from \citep{speagle2019DynestyDynamic}. This describes how the evidence integral in Eq.~\ref{searchcw:bayes:nested:evidence} can be approximated by sampling over increasingly smaller prior masses. \label{searchcw:bayes:nested_alg}}
\end{algorithm}
%
