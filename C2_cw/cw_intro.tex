...
This is done by using the prior mass
\begin{equation}
\label{searchcw:bayes:nested:priormass}
X(\lambda) = \int_{\theta: \mathcal{L}(\theta) > \lambda} \pi(\theta) d\theta,
\end{equation}
where $\pi(\theta) = p(\theta \mid I)$ is the prior. This the amount of the prior where the likelihood $\mathcal{L}(\theta)$ is greater than some value $\lambda$.
...
\end{equation}
where $\mathcal{L}(X)$ is the likelihood where $P(\mathcal{L}(\theta) > \lambda) = X$. One can find how we can get to this integral in Appendix \ref{appb}.
...
In Fig.~\ref{cwinto:bayes:nestedsampling:plots} the prior mass is shown for a given value of $\lambda$ as the blue shaded area in the first panel, where the limits are defined by the likelihood.
...
\begin{equation}
\label{intro:bayes:nested:evidence_trapz}
Z \approx \hat{Z} = \sum_{i=1}^{M} \mathcal{L}_i \left[  X_{i-1} - X_{i}\right].
\end{equation} 
However, $\mathcal{L}(X)$  is typically not known.
...
One could take $K$ samples (or live points) of $\theta$ from the prior distribution $\pi(\theta)$, then calculate the minimum value of the likelihood for these values $\mathcal{L}_{\rm{min}}$ and save the values of the parameters $\theta_{\rm{min}}$ associated with this likelihood.
Then replace the sample $\theta_{\rm{min}}$ with a new sample from the prior distribution with the constraint that $\mathcal{L}(\theta) > \mathcal{L}_{\rm{min}}$ and repeat the above steps $M$ times.
This then records samples after each step such that the values of $\mathcal{L}_{\rm{min}}$ are increasing, and therefore $X$ is decreasing such that
\begin{equation}
	0 < X_M < ... < X_1 < X_0 = 1.
\end{equation}
We could then use the values of $\mathcal{L}_{\rm{min}}$ with Eq.~\ref{intro:bayes:nested:evidence_trapz} to find the evidence is ce calculated the prior mass for each step $X_i = P(\mathcal{L}(\theta) > \mathcal{L}_{\rm{min},i})$.
However, rather than doing this calculation the value of $X_i$ can be approximated after each step by setting $X_i = E[X_i]$ where 
\begin{equation}
E[X_i] \approx \exp{\left(-\frac{i}{K}\right)}.
\end{equation}
This tells us that using this algorithm the log-prior mass shrinks by a factor $\sim -1/K$ after each iteration \citep{speagle2019DynestyDynamic}. 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE{ $\mathcal{L}^{\rm{min}} = \rm{min}\left\{\mathcal{L}(\theta_1),...\mathcal{L}(\theta_{K})\right\}$} \COMMENT{find minimum likelihood values for all live points}
		...
		\WHILE{K > 0}
			\STATE{$\mathcal{L}^{\rm{min}} = \rm{min}\left\{\mathcal{L}(\theta_1),...\mathcal{L}(\theta_{K})\right\}$} \COMMENT{compute minimum likelihood of current live points}
			\STATE{$D_i  = \theta_k$ } \COMMENT{add the live point $\theta_k$ associated with $\mathcal{L}^{\rm{min}}$ to list of `dead' points $D$.}
			\STATE{remove $\theta_k$}
			\STATE{$K = K - 1$}
		\ENDWHILE
...
The integral in Eq.~\ref{searchcw:bayes:nested:evidence} can then be approximated using Eq.~\ref{intro:bayes:nested:evidence_trapz},
\begin{equation}
	Z = \int_0^1 \mathcal{L}(X) dX \approx \sum_{i=1}^{i=M}  \mathcal{L}(\theta_i)  \left[ X_{i-1} - X_{i} \right] = \sum_{i=1}^{M} p(\theta_i),
\end{equation}
where $M$ is the number of iterations in Alg.~\ref{searchcw:bayes:nested_alg}, $p(\theta_i)$ is the importance weight defined as $p(\theta_i) =  \mathcal{L}(\theta_i) \left[ X_{i-1} - X_{i} \right]$. The likelihood values $\mathcal{L}(\theta_i)$ is calculated for each dead point $D_i$ with its associated change in prior mass $\Delta X = \left[ X_{i-1} - X_{i} \right] \approx -i/K$. 
