...
All of the above tests, test the search in the range from 100-200 Hz which is within \acp{LIGO} most sensitive band. 
To see how this search performs at other frequencies, we tested the same parameters and search at 100,250,500,750,1000,1500,2000 Hz.
Fig.\ref{viterbi:sens:results} shows how the sensitivity of the search drop as the frequency increases.
This is perhaps unfair to the algorithm as we used the same setup of the search for each of the different frequencies.
Each sub-band which was search over was 0.1 Hz wide, however, at frequencies above 500 Hz, the doppler shift of the signal means that the entire signal is not contained within a single frequency band.
This means that for each signal a large amount of the signal is not visible to the search.
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{C3_soap/snr_with_freq.pdf}
	\caption{The sensitivty of the SOAP search in this configuration decreases as the frequency of the pulsar increases. This setup of data for the search was optimised for the 100-200 frequency band.}
	\label{viterbi:sens:results}
\end{figure}
