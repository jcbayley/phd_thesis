...
There were $ $ \joe{fill} sets of parameters which were at this minimum, which along with the distribution in Fig.~\ref{soap:las:optimisation:s6mdc} demonstrates how the sensitivity if insensitive to the choice of parameters within some range.
\joe{need to say a bit more here}
...
The lines labelled Line-aware and Old Line-aware refer to parameters in Eq.~\ref{soap:las:optimisation:s6_pars} and Tab.~\ref{soap:results:parameters} respectively. 
...
Figure \ref{soap:las:optimisation:vitexample} shows how the two statistics optimised for the S6 \gls{MDC} perfom similarly on a specific example.
