...
%\epigraph{I've had it with these motherf****g snakes on this motherf****g plane}{ --- \textit{Samuel Jackson}, Snakes on a Plane}
...
\end{equation}
%
where $F_j$ is the frequency domain power summed over sub-segments within a single day, as described in Sec.~\ref{soap:sumdata} and  $d$ is the number of degrees of freedom,  equal to twice the total number of summed \glspl{SFT}.  $M_{\rm{N}}$ represents the model that the data is simply Gaussian noise. In the presence of a signal (model $M_{\text{S}}$), the power should follow a non central $ \chi^2 $ distribution in which the non-centrality parameter $\lambda$ is the square of the \gls{SNR}, $(\lambda = \rho_{\rm{opt}}^2 )$, i.e.
%
\begin{equation}
...
\subsection{S6 \gls{MDC} injections}
...
To reduce the time taken for the optimisation, the tests were run for each set of parameters in the 100-200 Hz frequency band.
Figure \ref{soap:las:optimisation:s6mdc} shows the projections in
parameter space for each of the parameters where the values are the \glspl{SNR} at 90\% efficiency.  
These projections are made by taking the mean across the other parameters.
...
Figure \ref{soap:las:optimisation:s6mdc} demonstrates that changing this
parameter does not have much effect on the final sensitivity. 

To find the optimal set of parameters, the global minimum of the parameter space is found. 
There were $\sim 1000$ sets of parameters which were at this minimum, which along with the distribution in Fig.~\ref{soap:las:optimisation:s6mdc} demonstrates how the search insensitive to the choice of parameters within some range.
This set of parameters covers the majority of the range which was optimised except for values of $w_{\rm{L}} < 4$.
Some examples are shown in Tab.~\ref{soap:las:optimisation:opttable}.
%
\begin{table}
	\centering
	%
	...
	\begin{tabular}{c c c c c}
		...
		\hline
		 & $\tau$ & $w_S$ & $w_L$ & $p(M_{\rm{L}})/p(M_{\rm{G}})$ \\
		\hline
		1 & 1.005& 10.0& 15.78& 0.0549\\
		\hline
		2 & 1.01& 3.0& 7.34& 0.0954\\
        \hline
        3 & 1.0& 9.0& 13.67& 0.1657\\
        \hline
        4 & 1.0& 2.0& 11.56& 0.0105\\
        \hline
        5 & 1.01& 4.0& 20.0& 0.1657\\
        \hline
	\end{tabular}
	\egroup
\end{table}
...
In general the sets of parameters had the structure that $w_{\rm{S}} < w_{\rm{L}}$ and as $w_{\rm{L}}$ increased the ratio $p(M_{\rm{L}})/p(M_{\rm{G}})$ increases. 
This implies that there is a larger fraction of high \gls{SNR} lines within the data which this statistic can suppress. 
The parameters space was flat in $\tau$ in the chosen range.
This all implies that if parameters are chosen somewhere near the global minimum area, the search will perform sufficiently well.
%
\begin{figure}[p]
    \centering
...
\end{figure}
%
To visualise how these optimised parameters perform compared to the values used in
Sec.~\ref{soap:results} we can test how they perform on a simulated signal
...
signal is weak. 
This is demonstrated in the viterbi map in the fourth panel of Fig.~\ref{soap:las:optimisation:vitexample}, the log-odds becomes dominated by the instrumental line. Whilst the astrophysical signal is still visible for parts of the spectrogram, the line dominates.
The S6 optimised statistic looks for more consistent \gls{SNR} in each of the detectors. 
The lines labelled "Line-aware" refers to the set of parameters in row $3$ of Tab.~\ref{soap:las:optimisation:opttable} and "Old Line-aware" refers to those in Tab.~\ref{soap:results:parameters}. 
This demonstrates how using the line-aware part of the statistic improves the robustness of the
algorithm against non-astrophysical signals.
Figure \ref{soap:las:optimisation:vitexample} shows how the two statistics optimised for the S6 \gls{MDC} perform similarly on a specific example.
However, Sec.~\ref{soap:las:optimisation:comp_sens} demonstrates how each set of parameters performs when tested on many examples. 
...
These three parameter sets are tested on S6 \gls{MDC} simulations between 40 and 500 Hz, which is a wider range than they were optimised for.
The process of running on the S6 \gls{MDC} and generating the efficiency curves is the same as in Sec.~\ref{soap:results}.
Figure \ref{soap:las:optimisation:comparison} shows the efficiency curves at a 10\% false alarm rate. 
...
There are other time-frequency representations such as the Q transform, which allow us to visualise and search for these signals. However, the bins Q-transforms are not independent, therefore the Bayesian formalism described in this chapter is no longer technically true.
Despite this, the SOAP search can still be run on the Q transform and return useful results, this is shown for GW150914 \citep{abbott2016ObservationGravitational} in Fig.~\ref{soap:sens:other:gw150914}.
...
SOAP is designed long duration signals where the \gls{SNR} can be build up over time, and a frequency track exists for the entire length.
...
Therefore, although short \gls{CBC} signals are not the ideal source for SOAP, what it can do is highlight areas which are more likely to contain a signal. 
...
Sections \ref{soap:lineawareamp}, \ref{soap:las:optimisation}, \ref{soap:sensfreq} and \ref{soap:sens:other} are additional to the published work in \citep{bayley2019SOAPGeneralised}.
...
This optimised the parameters of the line aware statistic in a case where the data does and does not contain instrumental artefacts.
This revealed that there are large areas of parameter space where the search performs at or close to its optimum. 
As the optimisation procedure is time consuming, this allows the same set of line-aware statistic parameters to be run on multiple observing runs without having to re-optimise the parameters. 
...
~\chris{You could add to this section with a discussion about the extra things
you've added in the Sec. 3.9, Sec. 3.11, Sec. 3.11.3 and Sec. 3.11.4.}
