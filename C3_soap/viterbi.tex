...
\caption{\joe{redo this caption aswell}This shows the line-aware statistic as a function of it input from each detector. This example is for parameters $p(\lambda,w_{\rm{S}}) = 4$, $p(\lambda,w_{\rm{L}}) = 0$ and $p(M_{\rm{L}})/p(M_{\rm{G}}) = 0$. So the line part of the statistic is not operating.So the line part of the statistic is not
...
These projections are made by taking the mean~\chris{if you're taking the mean then
...
log-odds along the path of the \gls{BNS} signal.~\chris{
