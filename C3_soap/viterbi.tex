...
For the majority of the analysis that follows the \glspl{SFT} are summed over one day, this will be explained in greater detail in Sec.~\ref{soap:sumdata}. 
...
Whilst SOAP was designed to search for sources of \glspl{CW}, when set up correctly, it can be applied to searching for many other types.
...
In previous SOAP searches the default length of \gls{SFT} has been 1800 s, this is not suitable when searching for \gls{CBC} signals.
We can take shorter time base for the \glspl{SFT}, however, to get a required resolution, these can overlap.
\joe{section not complete}
