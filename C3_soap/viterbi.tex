...
All of the above tests~\chris{grammar and be more specific - which tests
exactly?}, the search was conducted in the range from 100-200 Hz.
...
Fig.\ref{soap:sens:results}~\chris{don't start sentence with abreviation} shows the resulting efficiency curves from each of these tests.
...
Optimising the search means,~\chris{not where a commas should go} choosing the parameters of SOAP, the key parameter which will affect this is the transition matrix. 
...
The current setup of the search does not allow this size of jump, \chris{and} therefore, would struggle to identify this type of track.
The other main factor which will decrease this sensitivity is the sub-band width of 0.1 Hz. 
Similarly to before~\chris{clunky}, as the frequency increases the scale of the
Doppler modulation will increase as~\chris{shown} in Eq.~\ref{soap:results:dforbit}.
...
frequency~\chris{Don't step back from your work. This sentence sounds like,
"hey, I did a test but it's not that good, it's only a small test, nothing to
see here. Just state the test, its limitations and the conclusions you've drawn
from it. Simple.}.
...
%%%%%%%%%%%%
%%%%%%%%%%%%%%
\subsection{\label{soap:sens:other} Searching for non~\chris{hyphen?}CW sources}
%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
...
searched for using SOAP~\chris{I agree but techncailly it should also have the
property of living on a single track (some signals do not) and it should
ideally span the time length of the spectrogram (i.e., be continuous in some
way}.
...
s~\chris{they actually last $\sim$100s after entering our band. The length
depends only on the low frequency sensitivity and what we class as the lowest
frequency.}. 
...
~\chris{However, it does also mean that the data is not independent and our
Bayesian formulation is now not correct in the strcitest sense.}
...
~\chris{well power is in the SFTs. In the Vitmap we actually have log prob so
maybe a more specific statement is suitable.}where the signal lies.
...
$\sim 20$ and $\sim 520$ Hz~\chris{why is this approximate? You made the plot
so you know the exact bounds.}, however the SOAP search returns the same track
for a wider band-width up to 2000 Hz.~\chris{Did Lilli ever do the BNS analysis
with their Viterbi algorithm? Did it ever get published. If so, you should
reference it.} 
...
\citep{abbott2017GW170817Observation}. Each \gls{FFT} in the spectrograms are
0.2 s long and are overlapping by  0.189 s (95\%). This demonstrates that the
SOAP search can identify part of a \gls{BNS} signal within the data,
particularly the lower frequency parts. The \gls{BNS} signal here is the chirp
signal which ends at a time of 0. The Viterbi map shows a clear area of higher
log-odds along the path of the \gls{BNS} signal.~\chris{you are verging on
giving conclusions within the figure captions at the expense of actually
describing the figure. What are the red lines? What is shown in each panel?
Also, you might want to offest the red lines so that the SFT power can be seen.}}
...
investigation.~\chris{However, it's not impossible since there are TF
representations that show chirp-like structures. They don't use spectrograms
though. You could use different input data BUT you would have the issue of
independence of bins.}
...
For the two detector ~\chris{case} where two \gls{SFT} powers are summed,
\begin{equation}
...
\end{equation}
~\chris{You can't say that $T$ equals one thing and then say that $T$ equals
another thing. They are diferent $T$s so best to add a subscript to each of
them indicating what they apply to.}
...
This~\chris{ambiguous. What is "this"?} search
should take $\mathcal{O}(10^3)$ core-hours~\chris{Can you make the notation
match between statements - say thousands and millions or say $10^3$ and $10^6$}.
...
In this paper~\chris{have you decided on what you're doing about langiage like
this? Are you keeping "paper" or editing throughout to be more thesis chapter
like?} we describe an application of the Viterbi algorithm, called SOAP,
