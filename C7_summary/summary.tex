...
observing runs.~\chris{this sentence seems unconnected to the others}  For many
...
searching for \glspl{CW}, and also proved a non parametric way~\chris{grammar -
proved a non-parametric way?} to search for long duration signals.
...
Markov process.  We utilised~\chris{fancy use} this algorithm to search through
...
Specifically, the search is contaminated~\chris{is the search contaminated or
the data?} with instrumental lines which are long duration narrow band spectral
...
in each segment.  For each of these statistics there is a set of ~\chris{search
or hyper?} parameters which were optimised based on \gls{CW} signals simulated
in noise. 
...
Gaussian noise but~\chris{remove but} with temporal gaps corresponding to times
...
\gls{CW} searches sensitivity to isolated neutron stars~\chris{ref?}.  In this
test we achieved a sensitivity which is comparable to other \gls{CW} searches,
achieving a sensitivity~\chris{repetitious} depth of $\sim 13 \,
...
signal types as it searches~\chris{repetition of search} for mostly un-modelled
...
would however require more investigation to determine~\chris{determine?
develop?} a reliable detection statistic.  SOAP still has limitations however,
...
be manually removed in the analysis,~\chris{bad comma - grammar is incorrect
here} where Chapter \ref{machine} aims to address this problem.
...
be~\chris{are?} discussed in Chapter \ref{par_est}. 
...
instrumental lines.  In Chapter \ref{soap}, we described how we~\chris{we or I
or neutral. You should try to make things as consistent as possible throughout.} had to
...
containing an instrumental line or not.~\chris{wasn't it a bit more than that.
Weren't you classifying between signal and not signal?}

Sec.~\ref{machine:nn}~\chris{stop starting sentences with abreviations. It
should be Section X} contains an introduction to how neural networks are
...
were~\chris{originally?} designed to identify features within an image, where
...
returns a desired value~\chris{expand slightly on what a desired value is}.
Training data-sets are generally very large,~\chris{bad comma} this allows the
...
\gls{CW} simulations~\chris{divided} into four data-sets: Gaussian noise and
...
This~\chris{this is a new paragraph so don't refer to "This". State clearly
what you are referring to} showed that applying a \gls{CNN} to the output
...
comparison to some~\chris{no need for the word "some"} other all-sky \gls{CW}
...
that~\chris{comparable to? within the spread of?} of other all-sky searches.
This search however, can run with a computational time orders of magnitude
faster~\chris{than the others?}. In Sec.~\ref{machine:results:timing} we
...
identify these lines.~\chris{replace fullstop withe "where"?}  We use a single
detector search, which uses a simple statistic which~\chris{multiple "which"s}
is the sum of the \gls{SFT} power along the Viterbi track.  We have searched
between 40 and 500 Hz for instrumental lines~\chris{in what data?}, generating
...
line.~\chris{you could explicitly mention that these non-detected tracks were
transient and therefore not apriori expected to be detected by SOAP.}
...
search~\chris{searches?} for \glspl{GW}, with a focus on the methods used to
...
~\chris{very nice summary - and very nice thesis. Wll done Joe!}
