...
If we repeat this for all injections then the distribution of these \joe{confidence intervals} should follow a uniform distribution if the marginal posteriors agree with the injected parameters.
To see if these distribution agree, one can then plot the cumulative distribution of the \joe{confidecne intervals} to get a p-p plot as shown in Fig.~\ref{}, this plot is read as what fraction of simulations fall within a given \joe{confidence interval}.
