...
reduced, decreasing their computational cost.~\chris{somewhere in this chapter
you need to remind the reader that the output of the multidetector analysis is
a single track and to explain what it actually represents.}
...
Sec.~\ref{intro:prob:bayes}. \joe{will explain more in the return of intro
chapter}

As an example of what this~\chris{it's a new paragraph so you sjpild scpecify
what "this" is} returns, we can first simulate a \gls{CW} signal with
parameters 
%
\begin{equation}
...
L1~\chris{do you not want to show these?}. The SOAP search from
chapter~\chris{I think chapter should be capitalised like Section and Equation}
\ref{soap} is then run using the line-aware statistic~\chris{with?} the same
parameters as ~\chris{gven} in Tab.~\ref{soap:results:parameters}~\chris{be
consistent with you abbreviations - check that you using the same scheme
throughout}, the output Viterbi track is then plotted with the \gls{CW}
frequency track in Fig.~\ref{par_est:results:freqtrack}~\chris{you may want to
remind the reader that you only get 1 track out of a multidetector detector analysis}. 
%
\begin{figure}[ht]
    \centering
...
frequency~\chris{track?} using parameters in Eq.~\ref{par_est:results:examplepars} and the
...
Earth's orbit.~\chris{For reproducibility what is the actual start time of this
analysis (GPS).}} \label{par_est:results:freqtrack}    
\end{figure}
%
...
definition of the likelihood, however~\chris{not really a place for a however
"the results look wrong, however I will describe this later" - a however would
be used like "the results look wrong, however I will correct this later"}, I
...
bimodal.~\chris{this isn't a "but" statement. Things can be bimodal and in this
case the truth lies inside one of the modes so all is good. If you are to make
any special statement is should be on the SNR. Also what are the confidence
levels in your contours - please state these.}}
...
are shown on a sky projection in the ecliptic frame~\chris{grammar}. The sky
...
interpret.~\chris{repetative language :easier to interpret" - clean this
paragraph up} The Viterbi tracks and \gls{CW} frequency tracks used in this
...
$\alpha^{\rm{ecliptic}},\delta^{\rm{ecliptic}}$~\chris{be consistent - you used
$\gamma,\beta$ in the plot and the text} of a signal with \gls{SNR} 144. The
overlaid panel is a zoomed area around the posterior distribution, where the
orange marker shows the injected parameters.~\chris{no need to have the legend
in the plot. Also what are your contour definitions? You should state these in
all posterior plots}} \label{par_est:results:example_skypos}   
\end{figure}
%
...
%
%
To test this~\chris{what method? This is a new section!} method, we generate a
set of simulations in Gaussian noise, where we generate~\chris{multiple
"generate"} spectrograms of \gls{CW} signals as in Sec.~\ref{soap:results} and
...
$\alpha, \delta, f, \dot{f}$~\chris{what about SNR?} associated with each
...
The p-p plot~\chris{You're using P-P and p-p when you write p-p plot - choose
one. Also, since p is a probability you could use $p$--$p$ plot.} is a
...
normal distribution~\chris{what is this about a normal distribution? You mean
uniform}, then $P(\bm{q}_{\theta} > C) = C$ \citep{cook2006ValidationSoftware}.
...
If the analysis is valid, i.e the marginal posterior distributions agree with
the injected parameters~\chris{that's not the definition of a working analysis
- the real argument is that if the p-p plot is diagonal then the Bayesian
  probabilities can be trusted}, then this plot should follow a straight
...
the lower tail of the posterior for each~\chris{doesn't have to be for each
simulation - could just be a random trend in one direction with most posteriors
(not all) shifted up.} simulation, then the p-p plot will show a curve above
the diagonal.  Similarly, if the marginal posterior distribution is shifted to
left~\chris{same as previous comment} of the injected parameters, shown in the
second panel of Fig.~\ref{par_est:results:ppplot_example}, i.e. the true value
lies in the upper tail of the posterior for each~\chris{not each} simulation,
...
perfectly~\chris{again, not quite true - it can be wider then the perect
posterior but to under constrain it's central value is not as scattered as the
width would suggest} recovered posterior, then the curve will follow an S shape
...
narrower than the perfectly recovered posterior~\chris{not quite true. It could
be broader but it's central value would be scattered more broadly than it's
width would suggest}, then the curve will follow an S shape where the S is
above the diagonal at $C < 0.5$ and below the diagonal at $C> 0.5$.~\chris{the
p-p plot doesn't compare your results with the perfect posterior - it simply
asks wherether your posteriors are consistent. The perfect posteriors would be
diagonal AND have the smallest credible intervals.}
%
\begin{figure}[ht]
    \centering
...
uniform distribution.~\chris{if possible you could make this a 2X2 plot since
it looks a bit too compact compared to other plots in the thesis.}}
\label{par_est:results:ppplot_example}
\end{figure}

Using this information~\chris{which information - this is a new paragraph}, we
...
and $\dot{f}$ we recover an over constrained~\chris{I would recommend
hyphenating over and under-constrained throughout this section} posterior
...
the true distribution~\chris{not necessarily}, and the sky position parameters
...
If we assume that the estimated posterior distribution is correctly
recovered~\chris{I would stop using language that compares to the "correct"
posterior. Here all you want to say is that we assume the posterior contours or
credible intervals are trustworthy}, i.e. it is not under constrained or
...
Fig.~\ref{par_est:results:sky_area_example}~\chris{clunky language}. If the
...
constrained~\chris{well we already know from the p-p plot that the sky is
...
method to extract the ~\chris{sky?} parameters. 
%
\begin{figure}[ht]
    \centering
...
similar.~\chris{begs the question as to what are the black contours? I think
they should be removed for clarity. To be fancy you could also plot the
ecliptic curve on here to show the mirriring of the modes - just a
suggestion.}} \label{par_est:results:sky_area_example}
\end{figure}
...
confidence and sky areas which contain the injected parameters~\chris{could be
more precise in your language describing this latter quantity.}.
%
\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{C5_parameter/sky_area_hist.pdf}
    \caption[p-p plot for the CW simulations]{~\chris{fix short caption
description here} A histogram is shown~\chris{strange language} of the sky area
...
through the injected parameter~\chris{super clunky description - tidy up}. The
...
deg$^2$.~\chris{again, super clunky. I don't know what you mean here. Also, the
plots seem a bit squashed together vertically - maybe remove the x-axis label
on the upper plot. It would be nice to share x-axis ranges but I see that that
may be difficult.}}
...
However, if we assume that the 95\% confidence sky areas are correct, we can
...
~\chris{OK, but I'm a bit lost. If you use your 95\% values then most of the
time you report a region of <45 degrees which is 1/1000th of the sky. BUT how
often does that actually contain the true location? Also for Einstein at home I
don't think that you will be replacing the first stage of their search. You
will simply be using their more powerful and computatioinally expensive
analysis to follow up on your events. Finally, maybe refer to an LVC follow up
and not EatH.}
...
In this chapter I~\chris{fine to I but be consistent throughout the thesis. The
papers actually set the precedent by using "we" I think.} describe a Bayesian
...
output from the SOAP search in chapter \ref{soap}~\chris{your results are not
output in a chapter. They are described in a chapter - be careful}. The aim of
...
such as Einstein@Home \citep{singh2016ResultsAllsky}~\chris{try another LVC
example}. 

In Sec.~\ref{par_est:bayes} the setup of the Bayesian network is described,
where the input is a Viterbi track which is described~\chris{multiple
described} in chapter \ref{soap}, and the output is a posterior distribution of
...
then~\chris{used to construct?} the likelihood of this method.  For each of the
...
distributions and the sky position parameters are under constrained~\chris{not
true and it depends on the confidence you are asking for}. However, when
looking at the sky position parameters more carefully, we find that the
distribution is bi modal where each mode is over constrained~\chris{I don't
...
time.~\chris{By what factor (roughly) would you have to expand the the region
in order to get the correct value 95\% of the time?}
