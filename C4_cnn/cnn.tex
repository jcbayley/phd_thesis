...
The algorithmic details are summarised in Chapter \ref{soap}. 
...
Sec.~\ref{machine:nn}, \ref{machine:cnn} and \ref{machine:training} we explain how \glspl{CNN} operate and how they are trained. Sec.~\ref{machine:cw} will describe how this is applied to a \gls{CW} search. 
...
The `line-aware' statistic reduced the effect of instrumental lines on the analysis, however is still contaminated by certain types of line.
...
Throughout this section I will summarise one machine learning technique known as a neural network, for more information see \citep{emmert-streib2020IntroductoryReview} for a recent review of machine learning techniques. 
...
A \gls{CNN} has a similar goal to a fully connected neural network; it is designed to take in data, identify different features within that data and classify what those features or combinations of those features mean.
...
If the data was passed and the weights were updated a single time, the loss may decrease but is likely not at a minimum.
...
\begin{figure}[p]
	...
	%
	\caption[Structure of \glspl{CNN} used in the search for \glspl{CW}.]{\label{machine:results:cnnlayout} The structure of the Viterbi map and
...
\begin{description}
	...
	sub-bands. These signal have an \gls{SNR} in the range 20-200. The \gls{SNR} range here is wider than the training set as a method to test how the trained networks performs on a wider range of \glspl{SNR}. Here we again have a
...
\gls{CNN} achieves a sensitivity in SNR of $\sim 102$ and sensitivity depth of
$\sim 11\;{\rm Hz}^{-1/2}$ with 95\% efficiency.
...
The timings shown in Tab.~\ref{timing:table} are for the S6 observing run where each section is run on a single \gls{CPU} or \gls{GPU}, however, in practice the generation of the data is generally run on multiple \glspl{CPU} on a computing cluster.
...
not. This would then remove the need to manually look through frequency bands
...
\gls{CNN} achieved a sensitivity of \gls{SNR} $ \sim 102$ and sensitivity depth
$\sim 11 \; \rm{Hz}^{-1/2}$ . We have shown that the SOAP + \gls{CNN} approach
