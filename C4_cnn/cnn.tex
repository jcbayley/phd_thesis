...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.9\columnwidth]{C4_cnn/activations.pdf}
...
The difference being that \acp{CNN} use additional types of layers which can help to reduce the size of the problem.
For explanations that follow, I will use \acp{CNN} in the context of classifying images into one of two classes.

In a standard neural network, every pixel of an image is used as an input pixel. Therefore, the ordering of these pixels does not matter (as long as they are consistent across training examples).
Often in images, one wants to identify a feature within the image, this feature could potentially be in any part of the image. 
...
To show the difference I have an example of a simple input image which has 9 pixels, this is shows in Fig.~\ref{machine:cnn:convlayer:input}.

A fully connected neural network would flatten this image and apply Eq.~\ref{machine:nn:neuron:equation} to the inputs.
A convolutional layer however, filters the image and output a filtered image of the same size(the image can be a different size it depends how the layer was set up).
...
Assuming we use the image in Fig.~\ref{} as input to this network, in the fully connected case one output neuron would have 10 parameters to vary.
In the convolutional neural network case, assuming a filter size of 2$\times$2, one has 5 parameters to vary.  This then outputs am image of 9x9 pixels. 
This can then reduce the size of the parameter space, and should be able to identify the important features in the image when trained.
...
\begin{figure}[p]
    ...
    \caption{Convolutional filters can be designed to `pick out' certain features within an image. In this simple example above, the first filter (filter 1) matches the diagonal line in that orientation better than filter 2. The output filtered image the exaggerates this filter. The coefficients of the filter i.e. $F_{m,n}$ in Eq.~\ref{machine:cnn:conv:equation}, in this are set to ones and zeros.
    These are the weights which are trained by the network. In this an cases that follow, to get the same size image in the output as the input, the image is padded with zeros. I this image it was necessary to pad above and to the right of the image.}
...
\item[Activation function] The activation function is generally kept the same for each of the layers, however this can be set here. The different types have been explained in Sec.~\ref{machine:nn:activation}.
...
For example, in Fig.~\ref{machine:cnn:maxpool:image}, the image is reduced by a 2$\times$2 max pooling layer.

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{C4_cnn/maxpool.pdf}
    \caption{Max pooling layers aim to reduce the size of am image whilst retaining important information within the original image. Above shows an example where a 2$\times$2 maxpooling layer is used. }
...
%%%%%%%%%%%%%%%%%
\section{\label{machine:nn:training}Training}
%%%%%%%%%%%%%%%%%
...
Showing the network the same training example \joe{say why this is helpful}
...
In Sec.~\ref{soap:reuslts} to reduce the effect of instrumental artefacts, each of the search sub-bands was analysed by eye to determine if a sub-band was contaminated. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\label{data} Data generation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{data:injections} Signal simulations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{data:augmentation} Augmentation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
