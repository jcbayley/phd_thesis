...
I will not go into any detail on Einsteins field equations.
However, I will mention that in a linearised theory, when perturbations to the metric tensor are assumed to be small, Einstein's field equations can be solved such that the solution is a plane wave. 
...
In empty space where $T_{\mu \nu} = 0$, the Einstein field equations can be found to give, 
\begin{equation}
    \Box h_{\mu \nu} = 0,
\end{equation}
which is a wave equation. 
This then has the general solutions,
\begin{equation}
    h_{\mu\nu} = A_{\mu\nu}e^{ik_\alpha x^{\alpha}},
\end{equation}
where each component of $h_{\mu \nu}$ is a sinusiod travelling along vector $h^{\alpha}$ with amplitude $A_{\mu\nu}$.
This can be greatly simplified by choosing a different gauge (coordinate system) where the metric perturbation is both transverse and traceless (TT).
...
\begin{equation}
    ...
    \end{split}
\end{equation}
The affect of these two polarisation's on a ring of test particles can be seen in Fig.~\ref{gw:polarisations}.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{C1_Introduction/sources_types.pdf}
...
\acp{CBC} originate from the in-spiral and coalescence of compact objects. 
This compact objects generally refer to either black holes of neutron stars.
\ac{BBH} and \ac{BNS} signals are currently the only sources which have been observed by the \ac{LIGO} and Virgo detectors \cite{Abbott2017a,Abbott2017b,Abbott2016}. 
The general structure of the waveform follows a `chirp' which is shown in Fig.~\ref{sources:signaltypes}.
The lengths of the observed signal is generally dictated by the masses of the components of the system. 
Higher mass systems such as \ac{BBH} will be observed for less time in ground based detectors. This is because they are close to merger when they enter the detectors sensitive band.
...
This continues until the objects begin to merge. This is when the 
After the objects have merged the remnant compact object can 'ring down'. This means that the object still emits some gravitational waves as it settles into its final state.
