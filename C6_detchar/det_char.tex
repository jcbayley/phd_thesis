...
an instrumental line present within the sub-band.  We assume that statistics which are from Gaussian noise spectrograms follow a Gaussian distribution.
...
\ref{detchar:soap:lineplot} and \ref{detchar:soap:wanderplot} to investigate each sub-band and return a list of potential line. 
...
Whilst SOAP did not identify all of the same lines as the methods in Sec.~\ref{detchar:monitor}, it did identify a $\sim 150$ lines which did not appear in either the known of unknown line-list.
...
Whilst many of these will require further investigation to determine if they are a line and what their source is, it demonstrates the ability of this search to identify new instrumental lines.
