...
to search for \gls{CW}~\chris{plural}.  One of the major factors that
limited~\chris{limits} the sensitivity
of the search, is the presence of instrumental lines within the
data.~\chris{replace fullstop with "and"?}  Many of
...
identify the source or~\chris{do you mean "of"?} a line. 
...
sum of the \gls{SFT} power along the frequency track.~\chris{is this the only
mention of the actual statistic used? If so I recommend defining it
mathematically or referring to a previous relevent expression from Chapter 3.}  
...
Viterbi statistics are larger than this value.~\chris{the legend isn't quite
right. the red line itself doesn't represent the top 10\% - just the threshold} }
\label{detchar:soap:rankedstats}

\end{figure}
%
This shows that the majority of sub-bands~\chris{sub-band results} fall within
a single~\chris{be careful here - what is a single distribution? You're trying
to say that there is long tail to this distribution caused by the presence of intrumental lines} distribution,
...
potential instrumental lines by taking the top 10\%~\chris{why 10\%?} of Viterbi statistics.
...
%
% Introduce the plots
The line search then~\chris{also rather than then?} outputs
plots~\chris{similar to those?} as shown in Fig.~\ref{detchar:soap:noiseplot},
...
would not~\chris{does not?} show up in the top 10\% of Viterbi statistics, I
...
present within this sub-band.~\chris{as well as not being chi-squared it is
also variable in time which is another indicator that something is afoot.}
%
\begin{figure}[hpt]
	\centering
...
compared to the noise case,~\chris{bad comma} this is an indicator some
real~\chris{instrumental} signal. The
...
in~\chris{bad grammar} Fig.~\ref{detchar:soap:noiseplot},
...
provide a list of potential lines.  We~\chris{have} generated a list of
potential instrumental lines for \glspl{LIGO} third observing run (O3) between
40 and 500 Hz~\chris{clunky sentence coming out of nowhere}.  This line list
...
known.  Known lines can be accessed at {\tt
\url{https://ldas-jobs.ligo-wa.caltech.edu/~evan.goetz/CW/O3aLines/H1/index.html}}
and unknown lines at {\tt
\url{https://ldas-jobs.ligo-wa.caltech.edu/~evan.goetz/CW/O3aLines/H1/Unidentified/index.html}}
or are both stored in a git repository {\tt
\url{https://git.ligo.org/CW/instrumental/aLIGO-lines-combs}}.  ~\chris{these
urls are pretty ugly and don't wrap correctly on the page. Maybe add them as
references?}

These line lists can be compared to the list of lines generated by SOAP to
compare~\chris{compared to compare?} its performance to existing techniques.
...
threshold from Fig.~\ref{detchar:soap:rankedstats}.  SOAP then detected 55\% of
the lines of known origin and 30\% of the lines where the origin is not known.
Of the lines of known origin, all~\chris{is it really all. If so, please make a
bigger deal about it.} of those which SOAP did not identify were
from ``Calibration line mixing'' or ``Calibration line non-linearity''
\joe{woudl be nice to have a reference, cant find much rn}, upon further
investigation of these frequency bands, we found that these lines appear as
...
struggles to find short duration signals as the signal is weak and it cannot
...
Of the unknown line list, we~\chris{we?} 30\% of those on the \gls{LIGO} line
list~\chris{multiple "line list"} were in a sub-band which had a Viterbi
statistic which crossed our detection threshold.  However, Viterbi statistics
...
line list~\chris{best hyphenate line-list throughout}, such as in Fig.~\ref{detchar:soap:unknown:4}, there is no indication
...
of a line in the other outputs of SOAP.~\chris{I don't understand this last
sentence. Can you clarify it?}
%
\begin{figure}[hpt]
	\centering
	\begin{subfigure}[h]{0.49\textwidth}
...
do~\chris{not?} have a set of training data with known labels. 
...
~\chris{the problem you are discribing is perfect for un-supervised machine
learning approaches where you don't have labesl but are asking the algorithm to
classify the data based on its inherent variation. You might want to mention that.}

Whilst SOAP did not identify all of the same lines as the methods in Sec.~\ref{detchar:monitor}, it did identify a large number \joe{put the number}of lines which did not appear in either the known of unknown line list.
